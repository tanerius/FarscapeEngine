...
namespace Farscape {
    ...
    static void LoadFarscapeRuntimeAssembly(const std::string& path)
    {
        ...
        s_CoreAssembly = LoadAssembly("assets/scripts/Farscape-ScriptCore.dll");
    ...
    void ScriptEngine::Init(const std::string& assemblyPath)
    {
        ...
        LoadFarscapeRuntimeAssembly(s_AssemblyPath);
    }
    ...
    static FieldType GetFarscapeFieldType(MonoType* monoType)
    {
        ...
        case MONO_TYPE_VALUETYPE:
        {
            ...
            if (strcmp(name, "Farscape.Vector2") == 0) return FieldType::Vec2;
            if (strcmp(name, "Farscape.Vector3") == 0) return FieldType::Vec3;
            if (strcmp(name, "Farscape.Vector4") == 0) return FieldType::Vec4;
        }
        }
    ...
    void ScriptEngine::OnInitEntity(ScriptComponent& script, uint32_t entityID, uint32_t sceneID)
    {
        ...
        // Retrieve public fields
        {
            ...
            while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL)
            {
                ...
                FieldType farscapeFieldType = GetFarscapeFieldType(fieldType);

                // TODO: Attributes
                MonoCustomAttrInfo* attr = mono_custom_attrs_from_field(scriptClass.Class, iter);

                auto& publicField = s_PublicFields[script.ModuleName].emplace_back(name, farscapeFieldType);
    ...
    const Farscape::ScriptModuleFieldMap& ScriptEngine::GetFieldMap()
    {
