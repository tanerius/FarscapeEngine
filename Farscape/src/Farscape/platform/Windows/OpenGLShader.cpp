...
#if UNIFORM_LOGGING
#define FS_LOG_UNIFORM(...) FS_CORE_WARN(__VA_ARGS__)
#else
#define FS_LOG_UNIFORM
#endif
    ...
    std::string OpenGLShader::ReadShaderFromFile(const std::string& filepath) const
    {
        ...
        {
            FS_CORE_ASSERT(false, "Could not load shader!");
        }
    ...
    std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
    {
        ...
        while (pos != std::string::npos)
        {
            ...
            FS_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute", "Invalid shader type specified");
    ...
    int32_t OpenGLShader::GetUniformLocation(const std::string& name) const
    {
        ...
        if (result == -1)
            FS_CORE_WARN("Could not find uniform '{0}' in shader", name);
    ...
    void OpenGLShader::CompileAndUploadShader()
    {
        ...
        for (auto& kv : m_ShaderSource)
        {
            ...
            if (isCompiled == GL_FALSE)
            {
                ...
                FS_CORE_ERROR("Shader compilation failed:\n{0}", &infoLog[0]);

                // We don't need the shader anymore.
                glDeleteShader(shaderRendererID);

                FS_CORE_ASSERT(false, "Failed");
            }
    ...
    void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, Buffer buffer)
    {
        ...
        FS_CORE_ASSERT(uniform->GetLocation() != -1, "Uniform has invalid location!");
        ...
        case OpenGLShaderUniformDeclaration::Type::STRUCT:
            ...
            break;
        default:
            FS_CORE_ASSERT(false, "Unknown uniform type!");
        }
    }
    ...
    void OpenGLShader::UploadUniformFloat(const std::string& name, float value)
    {
        ...
        if (location != -1)
            ...
            FS_LOG_UNIFORM("Uniform '{0}' not found!", name);
    }
