...
namespace Farscape {
    ...
    static void OpenGLLogMessage(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam)
    {
        ...
        case GL_DEBUG_SEVERITY_HIGH:
            FS_CORE_ERROR("[OpenGL Debug HIGH] {0}", message);
            FS_CORE_ASSERT(false, "GL_DEBUG_SEVERITY_HIGH");
            break;
        case GL_DEBUG_SEVERITY_MEDIUM:
            FS_CORE_WARN("[OpenGL Debug MEDIUM] {0}", message);
            break;
        case GL_DEBUG_SEVERITY_LOW:
            FS_CORE_INFO("[OpenGL Debug LOW] {0}", message);
            break;
    ...
    void RendererAPI::Init()
    {
        ...
        {
            FS_CORE_ERROR("OpenGL Error {0}", error);
            error = glGetError();
        }
